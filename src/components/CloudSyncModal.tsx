...
import { saveSQLiteDatabaseOnServer } from '../utils/sqliteClient';
...
export const CloudSyncModal: React.FC<CloudSyncModalProps> = ({
  ...
  const handleCreateNewSpreadsheet = async () => {
    ...
    try {
      const title = `Rekap Absensi Guru - ${settings.schoolName} (${settings.academicYear})`;
  ...
  const handleSyncToGoogleSheets = async () => {
    ...
    const confirmSync = window.confirm(
      `Ekspor ${attendanceRecords.length} data absensi & ${students.length} data guru ke Google Spreadsheet?`
    );
  ...
  // Handle Save to Cloud & Local SQLite database.db
  const handleSaveToCloud = async () => {
    setIsSaving(true);
    let sqliteMsg = '';

    try {
      // 1. Save to Cloud Sync (Firestore)
      ...
      // 2. Also save to local SQLite database.db in project folder
      try {
        const sqliteResult = await saveSQLiteDatabaseOnServer({
          students,
          attendanceRecords,
          settings,
          teachers,
        ...
        if (sqliteResult.success) {
          sqliteMsg = ` + SQLite (database.db) tersimpan di folder aplikasi`;
        }
      } catch (sqErr) {
        console.warn('SQLite auto-save note:', sqErr);
      }
      ...
      if (result.success) {
        ...
        onShowToast('Sinkronisasi Database Berhasil', result.message + (sqliteMsg ? ` (${sqliteMsg})` : ''), 'success');
      } else {
        onShowToast('Cloud Sync Gagal', result.message, 'error');
      }
    } catch (e: any) {
      setIsSaving(false);
      onShowToast('Sinkronisasi Gagal', e.message || 'Terjadi kesalahan.', 'error');
    }
  };
  ...
  const handleLoadFromCloud = async (e: React.FormEvent) => {
    ...
    if (result.success && result.payload) {
      if (
        confirm(
          `Memuat data dari Cloud (${result.payload.students.length} Guru, ${result.payload.attendanceRecords.length} Catatan Absensi)? Data lokal Anda akan diperbarui.`
        )
      ) {
        onRestoreData({
    ...
    reader.onload = (event) => {
      try {
        ...
        if (json.students && json.attendanceRecords) {
          if (
            confirm(
              `Restorasi database dari file JSON (${json.students.length} Guru, ${json.attendanceRecords.length} Catatan Absensi)?`
            )
          ) {
            onRestoreData({
            ...
            onShowToast(
              ...
              `Database guru & absensi berhasil dimuat dari file backup JSON.`,
              'success'
            );
            onClose();
          }
        ...
        {activeTab === 'sync' && (
          <div className="space-y-4">
            ...
            <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-2xl flex items-start gap-2.5">
              <div className="w-7 h-7 rounded-xl bg-emerald-600 text-white flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 shadow-2xs">
                <i className="fa-solid fa-database"></i>
              </div>
              <div className="space-y-0.5 text-left">
                <p className="text-xs font-bold text-emerald-950 flex items-center gap-1.5">
                  <span>Database Ganda Aktif</span>
                  <span className="text-[10px] px-1.5 py-0.2 bg-emerald-200 text-emerald-900 rounded font-semibold">
                    Cloud Firestore + SQLite .db
                  </span>
                </p>
                <p className="text-[11px] text-emerald-800 leading-snug">
                  Saat tombol sinkronisasi ditekan, sistem otomatis menyimpan data ke <strong>Cloud</strong> sekaligus menulis file lokal <strong>database.db</strong> ke folder penyimpanan aplikasi.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <span>{isSaving ? 'Menyimpan ke Cloud & database.db...' : 'Simpan / Sinkronkan Data Sekarang'}</span>
            </button>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/60 rounded-2xl p-4 text-xs text-indigo-900 dark:text-indigo-200 space-y-1">
              ...
              <p className="text-[11px] text-indigo-800/80 dark:text-indigo-300/80 leading-relaxed">
                File cadangan format JSON menyimpan seluruh data master guru & tenaga kependidikan, riwayat presensi harian, pengaturan sekolah, dan data akun. Anda dapat memulihkan (restore) data ini kapan saja melalui tab <strong>Restorasi</strong>.
              </p>
            </div>
