...
export const AutoAbsenteeModal: React.FC<AutoAbsenteeModalProps> = ({
  ...
  const handleMarkSingle = (studentId: string, status: AttendanceStatus) => {
    onAddManualAttendance(
      studentId,
      status,
      `Ditandai ${status} via Peringatan Guru Belum Hadir`
    );
    setSuccessAlert(`Status guru berhasil diperbarui menjadi ${status}.`);
  ...
  const handleMarkAllAlpa = () => {
    ...
    const confirmAction = window.confirm(
      `Apakah Anda yakin ingin menandai ${absenteeStudents.length} guru/PTK yang belum hadir sebagai "Alpa" hari ini (${selectedDate})?`
    );
    ...
    try {
      ...
      setSuccessAlert(`Berhasil menandai ${absenteeStudents.length} guru sebagai Alpa.`);
  ...
  const handleCopySummary = async () => {
    if (absenteeStudents.length === 0) {
      alert('Tidak ada guru yang belum hadir.');
      return;
    }

    const classLabel = selectedClass !== 'Semua' ? `Jabatan ${selectedClass}` : 'Semua Guru & PTK';
    let text = `📢 *LAPORAN GURU BELUM HADIR / ALPHA*\n`;
    text += `🏫 *${settings.schoolName}*\n`;
    text += `📅 Tanggal: ${selectedDate}\n`;
    text += `⏰ Batas Masuk: ${settings.lateCutoffTime} WIB\n`;
    text += `🏷️ Lingkup: ${classLabel}\n`;
    text += `👥 Total Belum Hadir: *${absenteeStudents.length} Orang*\n\n`;
    text += `*Daftar Guru:* \n`;

    absenteeStudents.forEach((s, idx) => {
      text += `${idx + 1}. *${s.name}* (NIP: ${s.nis}) - ${s.classRoom}\n   📞 Kontak: ${s.parentPhone || 'Belum ada nomor'}\n`;
    });

    text += `\n_Mohon kepala sekolah / admin piket membantu konfirmasi kehadiran._`;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-extrabold text-base sm:text-lg">Deteksi & Peringatan Guru Belum Hadir</h3>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-white text-rose-700 uppercase">
                  Auto-Flag
                </span>
              </div>
              <p className="text-xs text-rose-100">
                Peringatan otomatis setelah jam masuk ({settings.lateCutoffTime} WIB) & konfirmasi presensi
              </p>
            </div>
          </div>
          <button
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-50 dark:bg-slate-800/80 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                title="Salin rekap guru belum hadir"
              >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs">
            <div className="px-4 py-3 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                ...
                <span>Daftar Guru Belum Melakukan Presensi Hari Ini ({selectedDate})</span>
              </h4>
            ...
            {absenteeStudents.length === 0 ? (
              <div className="text-center py-12 px-4 space-y-2">
                ...
                <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">
                  Luar Biasa! Semua Guru Telah Terdata Hadir
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                  Tidak ada guru yang belum hadir untuk tanggal {selectedDate}.
                </p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 self-end sm:self-center shrink-0">
                        ...
                        <button
                          ...
                          title={hasPhone ? 'Kirim WA Konfirmasi ke Guru' : 'Nomor WA belum terdaftar'}
                        >
