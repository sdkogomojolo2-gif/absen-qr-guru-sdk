  ...
  return new Promise<string>((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Only draw white background if transparency preservation is NOT requested and not PNG
        if (!preserveTransparency && mimeType !== 'image/png') {
...
/**
 * Specifically tuned for school & government logos with transparency preserved (PNG, 300x300 max, ~25-45 KB)
 */
export async function compressLogoImage(fileOrSource: File | string): Promise<string> {
  if (typeof fileOrSource !== 'string' && fileOrSource.type === 'image/svg+xml') {
